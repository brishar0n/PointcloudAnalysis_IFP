...
##### NEW CODE BEGINS OVER HERE
    n_points = len(xyz)
    r_label = f"{radius}"

    # --- NEW OPEN3D SETUP ---
    print(f"  Building Open3D KD-tree for radius {radius}m...")
    ...
    # Iterate through points (Open3D's hybrid search is optimized for individual queries)
    for i in range(n_points):
        if i % 200_000 == 0:
            print(f"    Progress: {i:,}/{n_points:,} ({100*i/n_points:.0f}%)")

        # --- REPLACED: query_ball_point + manual capping ---
        # search_hybrid_vector_3d returns: [count, indices, squared_distances]
        k, indices, _ = tree.search_hybrid_vector_3d(xyz[i], radius, max_neighbours)

        if k < 3:
            continue

        # Extract points using the returned indices
        pts = xyz[indices]
        ...
        # Normal is eigenvector of smallest eigenvalue (index 0 for eigh)
        normal = eigvecs[:, 0]
        if normal[2] < 0:
            normal = -normal
        normals[i] = normal

    # --- DERIVED FEATURES REMAINS THE SAME ---
    λ1 = eigenvalues[:, 0]
    λ2 = eigenvalues[:, 1]
    λ3 = eigenvalues[:, 2]
    λ1_safe = np.maximum(λ1, 1e-10)
...
def compute_roughness(xyz: np.ndarray, radius: float, max_neighbours: int = 100) -> np.ndarray:
    """
    Roughness = distance from each point to the best-fit plane of its
    neighbours using Open3D for accelerated spatial searching.
    """
    n_points = len(xyz)
    roughness = np.zeros(n_points, dtype=np.float32)

    # --- OPEN3D SETUP ---
    print(f"  Building Open3D KD-tree for roughness (radius {radius}m)...")
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(xyz)
    tree = o3d.geometry.KDTreeFlann(pcd)
    ...
    for i in range(n_points):
        ...
        # Hybrid search: radius + max capacity cap
        k, indices, _ = tree.search_hybrid_vector_3d(xyz[i], radius, max_neighbours)
        ...
        # Eigendecomposition to find the normal (smallest eigenvalue direction)
        _, eigvecs = np.linalg.eigh(cov)
        normal = eigvecs[:, 0]
        
        # Distance from the query point to the local plane defined by the neighbors
        # Formula: |(Point - Centroid) · Normal|
        roughness[i] = np.abs(np.dot(xyz[i] - centroid, normal))
...
#             print(f'\twithin the chunk({start}): {i}/{length} ')
#             idx = start + i
#             if len(neighbours) < 2:
#                 continue
...
def compute_height_features(xyz: np.ndarray, radius: float = 1.0, max_neighbours: int = 200) -> dict:
    """
    Height-based features relative to local 2D neighbourhood using Open3D.
    """
    n_points = len(xyz)
    z = xyz[:, 2]

    # --- OPEN3D 2D SETUP ---
    # We project points to a 2D plane (Z=0) for the search
    print(f"  Building Open3D 2D KD-tree for height features (radius {radius}m)...")
    xyz_2d = xyz.copy()
    xyz_2d[:, 2] = 0 
    
    pcd_2d = o3d.geometry.PointCloud()
    pcd_2d.points = o3d.utility.Vector3dVector(xyz_2d)
    tree = o3d.geometry.KDTreeFlann(pcd_2d)

    height_above_min = np.zeros(n_points, dtype=np.float32)
    height_range = np.zeros(n_points, dtype=np.float32)
    height_std = np.zeros(n_points, dtype=np.float32)

    print(f"  Computing height features for {n_points:,} points...")

    for i in range(n_points):
        if i % 100_000 == 0:
            print(f"    Progress: {i:,}/{n_points:,} ({100*i/n_points:.0f}%)")

        # Search in 2D space (using the projected 2d point)
        k, indices, _ = tree.search_hybrid_vector_3d(xyz_2d[i], radius, max_neighbours)

        if k < 2:
            continue
            
        z_neighbours = z[indices]
        
        # Calculate features based on original Z values
        z_min_local = z_neighbours.min()
        height_above_min[i] = z[i] - z_min_local
        height_range[i] = z_neighbours.max() - z_min_local
        height_std[i] = z_neighbours.std()

    # Global normalised height (remains the same)
    z_min, z_max = z.min(), z.max()
    normalized_z = ((z - z_min) / max(z_max - z_min, 1e-6)).astype(np.float32)
